...
if config.has_section('special_nodes'):
    ...
    for key, value in config.items('special_nodes'):
        ...
        if not key.isdigit():
            continue
        try:
            # Parse flexible formats. All special nodes are power-sensor buoys;
            # the legacy `has_power_sensor` flag (position 4) is accepted but ignored.
            # (1) node_id
            # (2) node_id = label
