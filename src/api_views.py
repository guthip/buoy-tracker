...
def get_all_gateways():
    ...
    for node_id, node_info in mh.nodes_data.items():
    ...
    for special_node_id, connections in mh.special_node_gateways.items():
...
def get_gateway_connections(special_node_id: int = None):
    ...
    if special_node_id is not None:
        return mh.special_node_gateways.get(special_node_id, {})
    
    return mh.special_node_gateways
...
def get_special_node_packets(node_id=None, limit=50):
    ...
    if node_id is not None:
        packets = mh.special_node_packets.get(node_id, [])
        if limit is None:
            return list(packets)
    ...
    result = {}
    for nid, packets in mh.special_node_packets.items():
        if limit is None:
