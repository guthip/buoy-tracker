...
(function(){
  ...
  console.log('[APP.JS] document.body:', document.body ? 'exists' : 'NOT FOUND');
  console.log('[APP.JS] document.body.dataset:', document.body.dataset ? 'exists' : 'NOT FOUND');
  console.log('[APP.JS] apiKeyRequired attr:', document.body.dataset.apiKeyRequired);
  console.log('[APP.JS] apiKey attr:', document.body.dataset.apiKey);
  console.log('[APP.JS] isLocalhost attr:', document.body.dataset.isLocalhost);
  ...
  console.log('[APP.JS] Final values: apiKeyRequired=' + apiKeyRequired + ', isLocalhost=' + isLocalhost + ', apiKey=' + (apiKey ? 'SET' : 'EMPTY'));
  ...
  function makeApiRequest(method, url, callback) {
    try {
      console.log('[API] Request ' + method + ' ' + url + ' (apiKey=' + (apiKey ? 'SET' : 'EMPTY') + ')');
      ...
      if (apiKeyRequired && apiKey) {
        console.log('[API] Adding auth header');
        xhr.setRequestHeader('Authorization', 'Bearer ' + apiKey);
      } else {
        console.log('[API] NOT adding auth header (apiKeyRequired=' + apiKeyRequired + ', apiKey=' + (apiKey ? 'SET' : 'EMPTY') + ')');
      }
      ...
      xhr.onreadystatechange = function(){
        if (xhr.readyState === 4){
          console.log('[API] Response ' + method + ' ' + url + ' -> ' + xhr.status);
  ...
  // Initialize modal and show if needed
  try {
    initApiKeyModal();
    if (apiKeyRequired && !isLocalhost && !apiKey) {
      showApiKeyModal();
    ...
    console.error('[INIT] Error initializing API key modal:', e);
  }
})();
